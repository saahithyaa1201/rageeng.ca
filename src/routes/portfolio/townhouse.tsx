...
export const Route = createFileRoute('/portfolio/townhouse')({
  component: TownhousePage,
});
...
const TOWNHOUSE_PROJECTS = [
  {
    ...
    location: '2620 Brock Road, Pickering, ON',
  ...
  },
  {
    ...
    scope: 'Mechanical & Electrical Engineering Design',
    status: 'Design Completed',
    location: '333 Ritson Road North, Oshawa, ON',
    mepType: 'Mechanical & Electrical',
    overview:
...
function TownhousePage() {
  ...
  const activeProject = TOWNHOUSE_PROJECTS[selectedProjectIndex];
      ...
      <main className="bg-white min-h-screen text-ink pb-24">
        ...
        <div className="container-page px-6 lg:px-12">
          <div className="flex flex-wrap gap-3 border-b border-ink/10 pb-6 mb-12">
            {TOWNHOUSE_PROJECTS.map((project, idx) => (
              <button
                key={project.id}
