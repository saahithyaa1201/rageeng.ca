...
export const Route = createFileRoute('/portfolio/midrise-residential')({
  component: MidriseResidentialPage,
});
...
const MIDRISE_PROJECTS = [
  {
    ...
    location: '40 King Street West, Oshawa, ON',
...
function MidriseResidentialPage() {
  ...
  const activeProject = MIDRISE_PROJECTS[selectedProjectIndex];
      ...
      <main className="bg-white min-h-screen text-ink pb-24">
        ...
        <div className="container-page px-6 lg:px-12">
          <div className="flex flex-wrap gap-3 border-b border-ink/10 pb-6 mb-12">
            {MIDRISE_PROJECTS.map((project, idx) => (
              <button
                key={project.id}
